...
1. **NO to "One-Size-Fits-All" Texturing**: I initially applied one texture to the whole GLB.
...
1. **NO to Manual Scaling**: I tried to fix alignment by manually setting scales/positions in JS.
...
1. **NO to Heuristic Fallbacks**: I added `if (name.includes("Cube"))` to handle Blender's generic names.
...
1. **NO to Lazy Refactoring**: I used `// ... logic here` placeholders in a file write.
    - *Correction*: "Please don't use '... this logic' when making edit changes."
    - *Lesson*: Always provide complete, functional source code.
1. **NO to Internal Import Maps**: I tried to map `core/` and `entities/` via `importmap` to "fix" the structure split.
...
1. **NO to "Summary First" Workflow**: I summarized changes multiple times before actually calling the tool to apply them.
