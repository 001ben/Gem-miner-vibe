...
1. **`pipeline/` (Asset Factory)**: The "Source of Truth" for assets. Contains Blender Python scripts and procedural texture generators.
1. **`tools/` (DX Tools)**: Developer tools like the Asset Viewer (`tools/viewer`) that verify assets in isolation.
1. **`assets/` (Output/Artifacts)**: The compiled `.glb` and configuration files.
...
1. **Compilation**: Exports a `.glb` and updates `catalog.json`.
1. **Consumption**: `BulldozerRenderer` loads the GLB and applies materials based on IDs.
...
1. **`load()`**: `BulldozerRenderer` fetches the GLB.
1. **`setup()`**:
    - Traverses GLB.
    - Identifies Tracks (extracts Curves).
    - Identifies Chassis/Parts (applies Materials).
1. **`update()`**: Syncs the root Mesh position/rotation to the Physics Body.
