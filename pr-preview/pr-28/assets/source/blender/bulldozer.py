...
def generate_body_texture(filepath):
    """
    Generates the procedural body texture and saves it as a PNG.
    Mimics the JS logic: Yellow base, Noise dots, Rust circles.
    """
    width = 512
    height = 512
    size = width * height
    ...
    def blend_pixel(x, y, r, g, b, a):
        if 0 <= x < width and 0 <= y < height:
            ...
            pixels[idx+3] = 1.0 # Keep alpha 1.0 for output

    # 1. Noise (1000 dots, 2x2)
    # JS: ctx.fillStyle = Math.random() > 0.5 ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.1)';
    for _ in range(1000):
        is_dark = random.random() > 0.5
        if is_dark:
            # Black 0.1
            nr, ng, nb, na = 0.0, 0.0, 0.0, 0.1
        else:
            # White 0.1
            nr, ng, nb, na = 1.0, 1.0, 1.0, 0.1
    ...
    # 2. Rust Spots (20 circles)
    # JS: rgba(100, 50, 0, 0.2) -> (0.39, 0.196, 0.0, 0.2)
    ...
    for _ in range(20):
        ...
        max_x = min(width, cx + radius)
        min_y = max(0, cy - radius)
        max_y = min(height, cy + radius)
    ...
    # Save Image
    image = bpy.data.images.new("BodyTexture", width=width, height=height)
    image.pixels = pixels
