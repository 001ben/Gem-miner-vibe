...
function createProceduralTexture(type) {
  ...
  if (type === 'tracks') {
    ...
    // Add Noise (Small dots for texture)
    ...
    return new Promise((resolve, reject) => {
      this.loader.load(url, (gltf) => {
        ...
        if (bodyMesh) {
          ...
          // enhanceMaterialWithTriplanar(matBody);
        ...
        const setupTrack = (points, side) => {
          ...
          // enhanceMaterialWithTriplanar(mat); // Disabled to prevent world-space projection artifacts
      ...
      setPose(position, angle) {
          this.group.position.set(position.x, this.scale * 1.0, position.y);
  ...
  setSpeeds(leftSpeed, rightSpeed) {
    ...
    this.animatedInstances.forEach(track => {
      ...
      destroy() {
          ...
          // Dispose of animated instances specifically if not caught above
          this.animatedInstances.forEach(track => {
              if (track.mesh) {
          ...
          try {
      this.animatedInstances.forEach(track => {
        if (!track.curve || !track.mesh) return;
        ...
        for (let i = 0; i < track.count; i++) {
          ...
          const tangent = track.curve.getTangentAt(t, this._tangent);
      ...
      if (!this._hasLoggedError) {
        console.error("Error updating bulldozer tracks:", e);
